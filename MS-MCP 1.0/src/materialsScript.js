...
export async function runMatScript(config, scriptFile, args = [], options = {}) {
  ...
  if (args.length > 0) runArgs.push("--", ...args.map(String));
...
export function buildMoleculeScript({ documentName, atoms = [], bonds = [] }) {
  const atomLines = atoms
    .map((atom, index) => {
      ...
      return `my $${label} = $doc->CreateAtom("${atom.element}", Point(X => ${Number(atom.x)}, Y => ${Number(atom.y)}, Z => ${Number(atom.z)}));`;
    })
    .join("\n");
  const bondLines = bonds
    .map((bond) => {
      const order = bond.order || "Single";
      return `$doc->CreateBond($${bond.from}, $${bond.to}, "${order}");`;
    })
    .join("\n");
...
my $doc = Documents->New("${documentName}");
${atomLines}
${bondLines}
$doc->CalculateBonds;
$doc->Save;
...
my $doc = Documents->Import("${inputDocument}");
my $results = Modules->Forcite->${task}->Run($doc, Settings(${settingsPairs}));
print "Forcite ${task} completed for ${inputDocument}\\n";
`;
}
...
print "CASTEP ${task} completed for ${inputDocument}\\n";
`;
}
