...
function ensureGuiAllowed() {
  if (!config.allowArbitraryScript) {
    throw new Error("GUI queue scripting is disabled. Set MS_MCP_ALLOW_ARBITRARY_SCRIPT=1 or do not set it to 0.");
  }
}
...
function readableInputPath(file) {
  return path.isAbsolute(file) ? path.resolve(file) : workspacePath(file);
}
...
eval {
    ...
    $doc = $opt;
    ${ballStickSnippet()}
    $doc->Export(${perlPathLiteral(exportPath)}, Settings(Version => "2023", OmitNewerContent => "No"));
    eval {
